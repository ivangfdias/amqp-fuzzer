...
unsigned char* extract_field_table(unsigned char* field_table, int base_index){
    /*
        ...
        while (i < table_size){
                ...
                snprintf(field_head, "%s%c%s\n%c", field_name, 0xca, field_type, 0xcb); //?
                switch(field_type){
                        case 'F':
                                // extract_field_table
                                // Esse é um ponto q provavelmente pode quebrar o broker por stack overflow porque provavelmente vai ser recursivo!!
                                field = extract_field_table(field_table);
                        case 't':
                                // boolean
                                // true = 1
                                // false = 0
